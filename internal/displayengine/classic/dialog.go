...
// (confirm, message, prompt, choice). View() and Layers() are kept on the outer type
// because they depend on the outer type's ViewString().
...
func RenderDialogBoxCtx(title, content string, dialogType DialogType, width, height, containerWidth, containerHeight int, ctx StyleContext) string {
	...
	dialogBox := InjectBorderFlags(borderStyle.Render(inner), ctx.BorderFlags, ctx.Border2Flags, true)
