...
func (m *MenuModel) ClearProcessingState() {
	...
	m.btnRow.Clear()
	// Recurse into content sections -- a plain *MenuModel container has no
	// other place to propagate this. A section's own spinner/processing
	// state would otherwise stay stuck after the outer dialog is
	// reactivated (e.g. returning from a confirm dialog), permanently
	// blocking that section's input.
