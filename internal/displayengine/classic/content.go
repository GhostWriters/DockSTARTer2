...
type Sizer interface {
	...
	// to be, given at most maxWidth -- used by calculateSectionLayout to
	// shrink a non-maximized MenuModel to intrinsic content width. Most
	// kinds just return maxWidth; the plain-list kind measures from its
	// items' Tag/Desc text.
...
type Content interface {
	...
	// type reaching updateSections, not just the ones its switch explicitly
	// handles (Tab/Shift-Tab, LayerHitMsg, wheel/drag/motion/release,
	// ToggleFocusedMsg, sinput clipboard messages). A *MenuModel reports true
	// only when it wraps a variable-height contentRenderer section whose
	// inner widget needs raw message types beyond those (e.g. a streaming
	// viewport forwarding PgUp/PgDn). A ContentRow reports true if any child does.
	WantsAllMessages() bool
	// Focusable reports whether this Content can receive Tab-cycled focus.
	// A *MenuModel reports false only for the plain-text kind (a read-only
	// display line Tab/Shift-Tab must skip); every other kind reports true.
	// A ContentRow reports true if any child is focusable.
	Focusable() bool
	// AbsorbMessage lets a Content section observe its own deferred-action
	// messages (button-row clicks, list-item Action clicks scheduled via
	...
	// this for every section on every message, not just the focused one, so
	// a section's deferred action still fires after focus has moved
	// elsewhere by the time the tick arrives. A ContentRow delegates to each
	// child.
	...
	// own Select-role button can spin too. A ContentRow reports true if any
	// child is processing.
	IsProcessing() bool
}
