...
func (m *AppModel) interactionWindow() time.Duration {
	interval := time.Duration(m.config.Appearance.ForConnType(m.connType).RefreshRate) * time.Millisecond
	if interval <= 0 {
...
// (the session's appearance refresh rate), so a coalesced drag or scroll never renders
