...
func newSetValueDialog(
	...
	tiStyles.Cursor.Color = displayengine.TextCursorColor()
...
func (m *setValueDialogModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		case m.focus == setValueFocusInput && displayengine.IsTypedText(msg):
		...
		case key.Matches(msg, displayengine.Keys.Left) && m.focus != setValueFocusInput:
			switch m.focus {
		...
		case key.Matches(msg, displayengine.Keys.Right) && m.focus != setValueFocusInput:
			switch m.focus {
			case setValueFocusSave:
				m.focus = setValueFocusCancel
			case setValueFocusCancel:
	...
	case displayengine.LayerHitMsg:
		...
		if msg.Button == tea.MouseLeft {
			...
			newOff, changed := displayengine.HandleScrollbarLayerHit("setvalue_preset_box", msg, m.offset, len(m.opts), m.maxVis)
			if changed {
				m.focus = setValueFocusList
				m.input.Blur()
				m.offset = newOff
...
func (m *setValueDialogModel) HelpContext(maxWidth int) displayengine.HelpContext {
	pageText := m.appDesc
	if m.varHelp != "" {
		if pageText != "" {
			pageText += "\n\n"
...
func (m *setValueDialogModel) ViewString() string {
	...
	currentValueSection := strings.TrimRight(displayengine.RenderBorderedBoxCtx(
	...
	if len(cvLines) > 0 {
		cvLines[len(cvLines)-1] = displayengine.BuildLabeledBottomBorderCtx(sInnerW+2, modeLabel, inputFocused, ctx)
	...
	buttonRow := strings.TrimRight(displayengine.RenderCenteredButtonsCtx(contentW, ctx, svBtnSpecs...), "\n")
	...
	return displayengine.RenderDialogWithTypeAndWidgets(title, lipgloss.JoinVertical(lipgloss.Left, parts...), m.focused || m.TitleBarFocused(), m.height, displayengine.DialogTypeInfo, m.State())
}
...
func (m *setValueDialogModel) Layers() []*lipgloss.Layer {
	return []*lipgloss.Layer{
		lipgloss.NewLayer(m.ViewString()).Z(displayengine.ZScreen).ID("setvalue_dialog"),
	}
}

func (m *setValueDialogModel) GetHitRegions(offsetX, offsetY int) []displayengine.HitRegion {
	ctx := displayengine.GetActiveContext()
	contentW := m.innerWidth()
	...
	regions = append(regions, displayengine.HitRegion{
		ID:     "setvalue_dialog." + displayengine.IDInsOvr,
	...
	sbInfo := displayengine.ComputeScrollbarInfo(len(m.opts), innerH, m.offset, innerH)
	m.lastSbInfo = sbInfo
	...
	regions = append(regions,
		displayengine.HitRegion{
			...
			ZOrder: displayengine.ZDialog + 25,
			Label:  "Help",
			Help:   &displayengine.HelpContext{ScreenName: m.setValueTitle(), PageTitle: "Help", PageText: "Open help for this dialog."},
		},
		displayengine.HitRegion{
			ID: "setvalue_dialog." + displayengine.IDTitleWidgetClose,
			X:  widgetsStartX + 4, Y: widgetY, Width: 3, Height: 1,
			ZOrder: displayengine.ZDialog + 25,
			Label:  "Close",
			Help:   &displayengine.HelpContext{ScreenName: m.setValueTitle(), PageTitle: "Close", PageText: "Close this dialog."},
		},
	)
