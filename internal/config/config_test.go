...
func TestLegacyMigration(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "ds2-migration-*")
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(tempDir)

	iniPath := filepath.Join(tempDir, "old.ini")
	iniContent := `
Theme='OldTheme'
Borders=no
Shadow=yes
...
`
	...
	conf, err := loadLegacyConfig(iniPath)
	if err != nil {
		t.Errorf("loadLegacyConfig failed: %v", err)
	}

	if conf.UI.Theme != "OldTheme" {
		t.Errorf("Expected Theme 'OldTheme', got '%s'", conf.UI.Theme)
	}
	if conf.UI.Borders != false {
		t.Errorf("Expected Borders false, got %v", conf.UI.Borders)
	}
	if conf.UI.Shadow != true {
		t.Errorf("Expected Shadow true, got %v", conf.UI.Shadow)
	}
	if conf.UI.ShadowLevel != 1 {
		t.Errorf("Expected ShadowLevel 1, got %d", conf.UI.ShadowLevel)
	}
}
