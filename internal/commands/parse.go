...
func Parse(args []string) ([]CommandGroup, error) {
	...
	for i < len(expandedArgs) {
		...
		case "-c", "--compose":
			if i < len(expandedArgs) && !strings.HasPrefix(expandedArgs[i], "-") {
				...
				validSubs := map[string]bool{
					...
					"create": true, "kill": true, "pause": true, "unpause": true, "rm": true,
		...
		case "--theme-tint-repo", "--theme-tint-file":
			if i >= len(expandedArgs) || strings.HasPrefix(expandedArgs[i], "-") {
				return nil, &ParseError{Args: expandedArgs, Index: i - 1, FailingCommand: cmd, Message: fmt.Sprintf("Command %s requires a connection type (local/ssh/web/all) and a %s.", cmd, map[string]string{"--theme-tint-repo": "scheme name", "--theme-tint-file": "file path"}[cmd])}
			}
			currentGroup.Args = append(currentGroup.Args, expandedArgs[i])
			i++
			if i >= len(expandedArgs) || strings.HasPrefix(expandedArgs[i], "-") {
				return nil, &ParseError{Args: expandedArgs, Index: i - 1, FailingCommand: cmd, Message: fmt.Sprintf("Command %s requires a %s.", cmd, map[string]string{"--theme-tint-repo": "scheme name", "--theme-tint-file": "file path"}[cmd])}
			}
			currentGroup.Args = append(currentGroup.Args, expandedArgs[i])
			i++
