...
func GetUsage(target string, noHeading bool) string {
	...
	if match("--tint", "--tint-list", "--tint-table") {
		printStr(
			...
			"	{{|UsageOption|}}menu{{[-]}} is DS2's own menus/dialogs/panels; {{|UsageOption|}}programbox{{[-]}} is a streamed command dialog's output; {{|UsageOption|}}cli{{[-]}} is a bare, non-interactive invocation, only ever meaningful for {{|UsageOption|}}local{{[-]}}. {{|UsageOption|}}cli{{[-]}} is silently skipped for ssh/web when the connection type is omitted or {{|UsageOption|}}all{{[-]}}, but an error if a connection type other than {{|UsageOption|}}local{{[-]}} is named explicitly alongside it.",
