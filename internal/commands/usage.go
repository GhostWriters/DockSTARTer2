...
func GetUsage(target string, noHeading bool) string {
	...
	if match("--server") {
		printStr(
			...
			"{{|UsageCommand|}}--server{{[-]}} {{|UsageOption|}}stop{{[-]}} [{{|UsageVar|}}pid{{[-]}}]",
			"	Stop the running server daemon. Specify a PID to stop a specific instance.",
			"{{|UsageCommand|}}--server{{[-]}} {{|UsageOption|}}restart{{[-]}} [{{|UsageVar|}}pid{{[-]}} [{{|UsageVar|}}sshPort{{[-]}} [{{|UsageVar|}}webPort{{[-]}}]]]",
			"	Stop and restart the server daemon. Optionally target a specific instance by PID",
			"	and override ports for the new instance.",
