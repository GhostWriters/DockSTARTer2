...
func GetUsage(target string, noHeading bool) string {
	...
	if match("--tint", "--tint-list", "--tint-table") {
		printStr(
			...
			"	List scheme names as {{|UsageOption|}}base16-{{[-]}}/{{|UsageOption|}}base24-{{[-]}}<scheme-name>, from the given source(s) (default: all; args may repeat, mix commas and spaces, and come in any order). More than one source prefixes each name \"<source>:\" to stay unambiguous. Every search word given must match a whole word (case-insensitive) in the slug, one word optionally {{|UsageOption|}}base16-{{[-]}}/{{|UsageOption|}}base24-{{[-]}} prefixed to also narrow by system (e.g. {{|UsageCommand|}}--tint-list{{[-]}} {{|UsageOption|}}repo:{{[-]}} {{|UsageOption|}}base24-ayu{{[-]}}).",
			"{{|UsageCommand|}}--tint-table{{[-]}} [{{|UsageOption|}}<repo:|user:|embedded:|all:>{{[-]}} ...] [{{|UsageOption|}}<search-word>{{[-]}} ...]",
			"	Show a table of schemes: slug, description, base16/base24 availability, from the given source(s) (default: all; args may repeat, mix commas and spaces, and come in any order). Every search word given must match a whole word somewhere in the scheme's metadata (slug, name, variant, or author -- e.g. {{|UsageCommand|}}--tint-table{{[-]}} {{|UsageOption|}}ayu dark{{[-]}} or {{|UsageOption|}}Kempson,light{{[-]}}). More than one source adds a Source column.",
		)
	}
