...
func printTableTo(w io.Writer, headers []string, data []string, useLineChars bool) {
	...
	for i, h := range headers {
		colWidths[i] = max(colWidths[i], utf8.RuneCountInString(ToPlain(h)))
	}

	// Check data
	for i, d := range data {
		col := i % cols
		colWidths[col] = max(colWidths[col], utf8.RuneCountInString(ToPlain(d)))
	}
	...
	printRow := func(rowItems []string) {
		...
		for i, item := range rowItems {
			visibleLen := utf8.RuneCountInString(ToPlain(item))
