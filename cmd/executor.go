...
func Execute(ctx context.Context, groups []CommandGroup) int {
	...
	for i, group := range groups {
		...
		case "-V", "--version":
			// Version Info
			fmt.Println(console.Parse(fmt.Sprintf("[_ApplicationName_]%s[-] [[_Version_]%s[-]]", version.ApplicationName, version.Version)))
			fmt.Println(console.Parse(fmt.Sprintf("[_ApplicationName_]DockSTARTer-Templates[-] [[_Version_]%s[-]]", paths.GetTemplatesVersion())))
			ranCommand = true
		case "-i", "--install":
